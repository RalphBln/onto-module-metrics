...
public class Cohesion {
  ...
  private HashSetValuedHashMap<OWLObject, OntologyModule> ontologyModulesByEntity =
  ...
  public Cohesion(OWLOntology baseOnto, Class<? extends OntologyModule> moduleClass) {
        ...
        .forEach(
            ontology -> {
              OntologyModule module =
                  OntologyModuleFactory.getInstance().createOntologyModule(moduleClass, ontology);
              ontology
    ...
    baseOnto
        ...
        .forEach(
            clazz -> {
              baseOnto
                  .axioms(clazz, Imports.INCLUDED)
                  .forEach(
                      axiom -> {
                        if (axiom.isOfType(Stream.of(AxiomType.SUBCLASS_OF))) {
                          OWLClassExpression superClassExp =
                          ...
                          if (superClassExp instanceof OWLClass) {
                            ...
                            ((OWLAnonymousClassExpression) superClassExp)
                                .classesInSignature()
                                .forEach(
                                    superClass ->
                                        processNonHierarchicalClassRelation(clazz, superClass));
                          }
                          ...
                          ((OWLEquivalentClassesAxiom) axiom)
                              ...
                              .forEach(
                                  equivalentClass ->
                                      processNonHierarchicalClassRelation(clazz, equivalentClass));
                        } else if (axiom.isOfType(AxiomType.DISJOINT_CLASSES)) {
                          ((OWLDisjointClassesAxiom) axiom)
                              .classesInSignature()
                              .forEach(
                                  disjointClass ->
                                      processNonHierarchicalClassRelation(clazz, disjointClass));
                        }
                      });
            });
    ...
    baseOnto
        ...
        .forEach(
            individual -> {
              baseOnto
                  .axioms(individual, Imports.INCLUDED)
                  .forEach(
                      axiom -> {
                        if (axiom.isOfType(Stream.of(AxiomType.CLASS_ASSERTION))) {
                          OWLClassExpression typeClassExp =
                          ...
                          if (typeClassExp instanceof OWLClass) {
                            ...
                            ((OWLAnonymousClassExpression) typeClassExp)
                                .classesInSignature()
                                .forEach(
                                    typeClass ->
                                        processNonHierarchicalClassRelation(individual, typeClass));
                          }
                          ...
                          ((OWLSameIndividualAxiom) axiom)
                              ...
                              .forEach(
                                  sameIndividual ->
                                      processNonHierarchicalClassRelation(
                          ...
                          ((OWLDifferentIndividualsAxiom) axiom)
                              ...
                              .forEach(
                                  differentIndividual ->
                                      processNonHierarchicalClassRelation(
                                          individual, differentIndividual));
  ...
  private void processHierarchicalClassRelation(OWLObject c1, OWLObject c2) {
    ...
    ontologyModulesByEntity.get(c1).stream()
        .forEach(
            sourceModule -> {
    ...
    // the other way round
    ontologyModulesByEntity.get(c2).stream()
        .forEach(
            sourceModule -> {
